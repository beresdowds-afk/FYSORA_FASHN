...
import { Eye, Mail, Bell, Lock, Crown, ShoppingBag } from "lucide-react";
...
export default function FreeTourConsentDialog({
  ...
  const handleStartTour = async () => {
    ...
    setSubmitting(false);
    onConsentGiven();
    // Stay on the catalogue — the parent renders read-only catalogue content.
    // The narrated /platform-tour flow is intentionally independent.
  };
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <div className="rounded-xl border border-border bg-card p-8 text-center max-w-md mx-auto">
          ...
          <h3 className="font-heading font-bold text-lg mb-2">Free Previews Exhausted</h3>
          <p className="text-sm text-muted-foreground mb-2">
            You've used all <span className="font-semibold text-foreground">{maxTours}</span> free catalogue previews.
          </p>
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="rounded-xl border border-border bg-card p-8 max-w-md mx-auto">
        <div className="text-center mb-6">
          ...
          <h3 className="font-heading font-bold text-lg mb-1">Free Catalogue Preview</h3>
        ...
        <div className="rounded-lg bg-muted/50 border border-border p-4 mb-6 space-y-3">
          ...
          <div className="flex items-start gap-2 text-sm text-muted-foreground">
            ...
            <span>Ordering, wishlists, and try-on remain <span className="font-medium text-foreground">disabled</span> until you subscribe</span>
          </div>
        </div>
        ...
        <p className="text-[10px] text-muted-foreground text-center mt-4">
          After {maxTours} free previews, a $10/year Premium subscription + identity verification is required for full access.
        </p>
      </div>
    </div>
  );
